...
function main
	...
	export AuxFunctions
	construct AuxCallback [repeat declaration_or_statement]
	export AuxCallback
	construct WalkCallbackIDs [repeat id]
	export WalkCallbackIDs
...
rule doSimpleTypeRule ModName [id] TPRULES [repeat type_rule_definition] TPDECS [repeat construction_assignment_statement]
	...
	construct callback [repeat declaration_or_statement]
		_ [genCallback OP LONG LONG] [genAnnotatedCallback OP LONG LONG]
	construct return [repeat declaration_or_statement]
		return true;
...
function genAnnotatedCallback OP [opt scl_additions] LONG [id] toPass [id]
	import noCallbackArg [number]
	deconstruct not noCallbackArg
		'1
	replace [repeat declaration_or_statement]
	...
	construct lowerLong [id]
		LONG[tolower]
	construct toPassLower [id]
		toPass[tolower]
	construct index [id]
		i
	construct passName [constant]
		NAME[tolower]'[index'].ptr.lowerLong
	construct valName [id]
		_ [+ "case "] [+ LONG] [+ "_VAL"]
	construct caseStmt [repeat declaration_or_statement]
		valName:
		callName(PARENT[tolower], '& passName, thePDU);
		freeName('&passName);
		break;
	import AuxCallback [repeat declaration_or_statement]
	export AuxCallback
    	AuxCallback [. caseStmt]
	by
		Stmts
end function
...
function initializeSetOfPointer mallocName [id] RuleName [id] LET [list element_type] ET [element_type] OP [opt scl_additions] 
	where not
		OP [checkForAnnotatedCallbackRemoval]
	deconstruct ET 
...
%		recursiveParse	
	...
	construct addedStmts [repeat declaration_or_statement]
		_ [checkForwardLengthConstraint mallocName functionName LONG SHORT OP]
			...
			[checkForAnnotatedCallbackWalk mallocName SHORT OP]
	by
		Stmts [. addedStmts]
end function
...
function checkForAnnotatedCallbackWalk mallocName [id] SHORT [id] OP [opt scl_additions]
	deconstruct OP
		EN [opt encoding_grammar_indicator] SZ [opt size_markers_block] TR [opt transfer_rules_block] CON [opt constraints_block] 
	deconstruct * [transfer_statement] TR
		'Callback '@ TYPE [id]
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	construct walkName [id]
		_ [+ "walk_"] [+ TYPE]
	construct callbackWalk [repeat declaration_or_statement]
		walkName (mallocName[tolower], SHORT[tolower], thePDU);
	construct freeWalk [repeat declaration_or_statement]
		if(SHORT[tolower] != NULL) {
			free(SHORT[tolower]);
			SHORT[tolower] = NULL;
		}
	...
	construct IDSet [repeat id]
		_ [. walkName] [. mallocName] [. mallocName] [. TYPE] [. SHORT] [. PDU] [. thePDU]
	import WalkCallbackIDs [repeat id]
	export WalkCallbackIDs
		WalkCallbackIDs [. IDSet]
	import FunctionDeclarations [repeat rule_definition]
	construct pModDecl [rule_definition] %[function_definition_or_declaration]
		bool walkName (mallocName[toupper] * mallocName[tolower], TYPE[toupper] * TYPE[tolower], PDU * thePDU);
	export FunctionDeclarations
		FunctionDeclarations [. pModDecl]
	by
		Stmts [. callbackWalk] [. freeWalk]
end function
...
function createCallbackWalkFunction
	import AuxCallback [repeat declaration_or_statement]
	where
		AuxCallback [checkIfEmpty]
	replace [program]
		P [program]
	import WalkCallbackIDs [repeat id]
	deconstruct [repeat id] WalkCallbackIDs
		FunctionName [id] mallocType [id] mallocName [id] TYPE [id] SHORT [id] PDU [id] thePDU [id]
	construct functionBody [repeat declaration_or_statement]
		_ [createCallbackWalk mallocName SHORT]
	construct FullFunction [repeat rule_definition]
		bool FunctionName(mallocType[toupper] * mallocName[tolower], TYPE[toupper] * SHORT[tolower], PDU * thePDU) {
			functionBody
		}
	import AuxFunctions [repeat rule_definition]
	export AuxFunctions
		AuxFunctions [. FullFunction]
	by
		P %[. FullFunction]
end function

function createCallbackWalk mallocName [id] SHORT [id]
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	construct index [id]
		i
	construct loopBound [constant]
		mallocName -> SHORT[tolower][+ "count"]
	construct loopBody [repeat declaration_or_statement]
		_ [callbackWalkBody index SHORT]
	construct callbackWalk [repeat declaration_or_statement]
		'uint32_t 'type;
		for(int index = 0; index < loopBound ; index++) {
			loopBody
		}
	by
		Stmts [. callbackWalk]
...
function checkIfEmpty
	match [repeat declaration_or_statement]
		Stmt [declaration_or_statement] Rest [repeat declaration_or_statement]
end function

function callbackWalkBody index [id] SHORT [id]
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	construct switchBody [repeat declaration_or_statement]
		_ [callbackSwitchBody index SHORT] [callbackSwitchDefault]
	construct switchStmt [repeat declaration_or_statement]
		'type = SHORT[tolower]'[index'].'type;
		switch ('type) {
			switchBody
		}
	by
...
function callbackSwitchBody index [id] SHORT [id]
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	import AuxCallback [repeat declaration_or_statement]
	%Construct definedCallbacks [repeat declaration_or_statement]
	%	_ [^ AuxCallback]
	construct switchCases [repeat declaration_or_statement]
		_ [callbackSwitchCases index SHORT each AuxCallback]
	by
		Stmts [. switchCases]
end function

function callbackSwitchCases index [id] SHORT [id] callback [declaration_or_statement]
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	by
		Stmts [. callback]
...
function callbackSwitchDefault
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	construct default [repeat declaration_or_statement]
		default:
...
function withoutLengthConstraintAnnotatedCallback mallocName [id] functionName [id] LONG [id] SHORT [id] OP [opt scl_additions]
	...
	replace [repeat declaration_or_statement]
		Stmts [repeat declaration_or_statement]
	construct freeMain [repeat declaration_or_statement]
		_ [checkMainAnnotatedCallback mallocName SHORT]	
	construct newStmts [repeat declaration_or_statement]
		int size = '0;
		mallocName -> SHORT[tolower][+ "length"] = thePDU ->curPos; 
		TYPE * SHORT[tolower];
		SHORT[tolower] = NULL;
		SHORT[tolower] = functionName(thePDU, '0 , &size, 'progname, endianness);
	construct endStmts [repeat declaration_or_statement]
		mallocName -> SHORT[tolower][+ "count"] = size;		
		mallocName -> SHORT[tolower][+ "length"] = thePDU ->curPos - mallocName -> SHORT[tolower][+ "length"]; 
	by
...
function freeTypeDecisionAnnotatedCallback RULES [repeat rule_definition] ALLOCNAME [constant] TYPE [id] LVAL [number] ET [element_type] previousET [element_type]
	...
	import FunctionDeclarations [repeat rule_definition]
	export FunctionDeclarations
		FunctionDeclarations [. FreeHeader]
	by
		Stmts %[. Stmt]
end function
...
function freeAltDecisionsAnnotatedCallback RULES [repeat rule_definition] ALLOCNAME [constant] LVAL [number] RTR [repeat alternative_decision] previousET [element_type]
	...
	export FunctionDeclarations
		FunctionDeclarations [. FreeHeader]
	by
		Stmts [freeAltDecisionsAnnotatedCallback RULES ALLOCNAME LVAL REST previousET] %[. Stmt] [freeAltDecisions RULES ALLOCNAME REST]
end function
