...
bool parsePIDTYPEMAXSIZESERIALIZED_O (PIDTYPEMAXSIZESERIALIZED_RTPS *pidtypemaxsizeserialized_rtps, PDU *thePDU, char *progname, uint16_t *type, uint8_t endianness);
...
bool parsePIDPARTICIPANTBUILTENDPOINTS_O (PIDPARTICIPANTBUILTENDPOINTS_RTPS *pidparticipantbuiltendpoints_rtps, PDU *thePDU, char *progname, uint16_t *type, uint8_t endianness);
...
bool parsePIDMETATRAFFICUNILOCATOR_O (PIDMETATRAFFICUNILOCATOR_RTPS *pidmetatrafficunilocator_rtps, PDU *thePDU, char *progname, uint16_t *type, uint8_t endianness);
...
bool parsePIDMETATRAFFICMULTILOCATOR_O (PIDMETATRAFFICMULTILOCATOR_RTPS *pidmetatrafficmultilocator_rtps, PDU *thePDU, char *progname, uint16_t *type, uint8_t endianness);
...
bool parsePING (PING_RTPS *ping_rtps, PDU *thePDU, char *progname, uint8_t endianness) {
    ...
    if (!(ping_rtps->header.protoname == 0x52545053 ||ping_rtps->header.protoname == 0x52545058)) {
        return false;
    }
...
bool parseFULL (FULL_RTPS *full_rtps, PDU *thePDU, char *progname, uint8_t endianness) {
    ...
    if (!(full_rtps->header.protoname == 0x52545053 ||full_rtps->header.protoname == 0x52545058)) {
        return false;
    }
...
bool parseDATASUB (DATASUB_RTPS *datasub_rtps, PDU *thePDU, char *progname, uint8_t endianness) {
    ...
    datasub_rtps->serializeddata = (unsigned char *) malloc (sizeof (unsigned char) * ((datasub_rtps->nextheader) ? datasub_rtps->nextheader - (serializeddataPOS - extraflagsPOS) : remaining));
    ...
    memcpy (datasub_rtps->serializeddata, &thePDU->data[thePDU->curPos], (datasub_rtps->nextheader) ? datasub_rtps->nextheader - (serializeddataPOS - extraflagsPOS) : remaining);
...
bool parsePIDRELIABILITY (PIDRELIABILITY_RTPS *pidreliability_rtps, PDU *thePDU, char *progname, uint8_t endianness) {
    ...
    pidreliability_rtps->slack = (unsigned char *) malloc (sizeof (unsigned char) * (pidreliability_rtps->parameterlength - sizeof (pidreliability_rtps->reliabilityqos)));
...
bool parsePROPERTY (PROPERTY_RTPS *property_rtps, PDU *thePDU, char *progname, uint8_t endianness) {
    ...
    if (!checkSlack (thePDU, (((thePDU->curPos - posname) % 4 ? 4 - ((thePDU->curPos - posname) % 4) : 0)))) {
        return false;
    }
    ...
    if (!checkSlack (thePDU, (((thePDU->curPos - posvalue) % 4 ? 4 - ((thePDU->curPos - posvalue) % 4) : 0)))) {
        return false;
    }
    return true;
}
